...
const de_YuFoLunChan = (str: string): string => {
    if (/[:：]/.test(str)) {
        str = str.replace(/(.*)(:|：)+(.*)/, "$3");
    }
    ...
    const cryptor = CryptoJS.AES.decrypt(base64Str, KEY, { 
        iv: IV, 
...
const is_YuFoLunChan = (str: string): boolean => {
    return /[滅苦婆娑耶陀跋多漫都殿悉夜爍帝吉利阿無南那怛喝羯勝摩伽謹波者穆僧室藝尼瑟地彌菩提蘇醯盧呼舍佛參沙伊隸麼遮闍度蒙孕薩夷迦他姪豆特逝朋輸楞栗寫數曳諦羅曰咒即密若般故不實真訶切一除能等是上明大神知三藐耨得依諸世槃涅竟究想夢倒顛離遠怖恐有礙心所以亦智道。集盡死老至冥奢梵呐俱哆怯諳罰侄缽皤]{5,}$/.test(str);
};

export {
    de_YuFoLunChan,
    is_YuFoLunChan,
    is_YuFoLunChan as detect,
    de_YuFoLunChan as decode
};
